    ...
    if( document.getElementById(element) != null ){
        ...
        if( optimizedDatabaseLoading == 1 ){
            google.maps.event.addListener(map, 'idle', function(){
                if( searchClicked != 1 ){
                    ...
                    loadData($("#submit_search").attr("data-ajax-data-file"), ajaxData);
                }
            });
        }
        else {
            google.maps.event.addListenerOnce(map, 'idle', function(){
                loadData($("#submit_search").attr("data-ajax-data-file"));
            });
        }
        ...
        function placeMarkers(markers){
            ...
            for (i = 0; i < markers.length; i++) {
                ...
                if ( markers[i]["location"]["lat"] && markers[i]["location"]["lng"] && markers[i]["location"]["formatted_address"] ){
                    renderRichMarker(i,"latitudeLongitude");
                }

                // Only Address

                else if ( markers[i]["location"]["formatted_address"] && !markers[i]["location"]["lat"] && !markers[i]["location"]["lng"] ){
                    renderRichMarker(i,"address");
                }

                // Only Latitude and Longitude

                else if ( markers[i]["location"]["lat"] && markers[i]["location"]["lng"] && !markers[i]["location"]["formatted_address"] ) {
                    renderRichMarker(i,"latitudeLongitude");
                }
            ...
            function renderRichMarker(i,method){
                if( method == "latitudeLongitude" ){
                    ...
                    var geoOptions = {
                        address: markers[i]["location"]["formatted_address"]
                    };
            ...
            function renderResults(){
                ...
                for (var i = 0; i < newMarkers.length; i++) {
                    if ( map.getBounds().contains(newMarkers[i].getPosition()) ){
	                    ...
	                    searchMarkersId.push("id:"+$(newMarkers[i].content.firstChild).attr("data-id"));
                ...
                if( $(".hero-section").hasClass("sidebar-grid") ){
                    ...
                    sidebarUrl = "api/admin/projects/sidebar_results";
                }	
				
                $.ajax({
                    url: sidebarUrl,
                    method: "POST",
                    data: { "ids":visibleMarkersId.join(";")},
                    success: function(results){						
