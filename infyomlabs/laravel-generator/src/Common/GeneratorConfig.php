...
//        $this->options['softDelete'] = config('infyom.laravel_generator.options.softDelete', false);
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        if(!empty($this->options['jsonFromGUI'])){
        	$json = json_decode($this->options['jsonFromGUI']);
        	if(!empty($json->iconName)){
		        $commandData->addDynamicVariable('$ICON_NAME$', $json->iconName);
	        }
        }
